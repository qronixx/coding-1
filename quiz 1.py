...
'1 - input() is a built-in function  that allows users to type data intoa a program and assign it to a variable'
'2 - print()'
...
'The difference between the two is that the single equal sign is used to assign values to variables. WHereas, the double equal signs meaning its comparing if the values are the same'

# 5. Write code that takes a user’s input (as a string), 
# casts it to a float, and prints the result multiplied by 2.

value = input('give me a decimal to be multiplied by 2')
print(2 * float(value))
...
'Function defintion is how we give or functions its value or purpose you could say and an invocation is us calling the fucntion so it can work.'

# 8. Why are functions useful in programming? Provide at least two reasons 
# and write your reasons in in complete sentences.

'Functions are important because it is how we prgram. Another reason it is important is because that is how we get our progamming/coding to show.'
